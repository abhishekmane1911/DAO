...
export default function ProposalDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const evs = await daoContract.queryFilter(
          daoContract.filters.ProposalCreated(),
          0,
          "latest",
        );
        const ev = evs.find((e) => Number(e.args.id) === Number(id));
        if (ev?.args?.description) {
          const m = await fetchProposalMetadata(ev.args.description).catch(
            () => null,
          );
          setMeta(m);
        }
  ...
  const vote = async (support) => {
    ...
    setTxLoading(true);
    try {
      // 🔥 ADDED: Pre-check voting power to save user gas
      const weight = await tokenContract.balanceOfAt(
        account,
        proposal.snapshotId,
      );
      if (weight === 0n) {
        setTxLoading(false);
        return toast.error(
          "You had 0 DGT at the time of snapshot and cannot vote.",
        );
      }

      const tx = await daoContract.vote(proposal.id, support);
      toast.loading("Submitting vote to Sepolia...", { id: "vote" });
      // ... rest of your code      await tx.wait();
      confetti({
        particleCount: 100,
        spread: 70,
        origin: { y: 0.6 },
  ...
  if (!proposal || proposal.id === 0) {
    return (
      ...
      >
        ...
        <Link to="/dashboard" style={{ color: "#f59e0b", fontSize: "13px" }}>
          ← Back to Dashboard
        </Link>
      </div>
    );
  ...
  return (
    <div
      style={{
        maxWidth: "800px",
        margin: "0 auto",
        padding: "clamp(80px,8vw,96px) clamp(16px,4vw,32px) 80px",
        position: "relative",
      }}
    >
      {/* Ambient glow */}
        ...
        <Section title="Current Votes">
          ...
          >
            <span style={{ color: "#34d399" }}>
              Yes — {yP.toFixed(1)}% &nbsp;(
              {yes.toLocaleString(undefined, { maximumFractionDigits: 0 })} DGT)
            </span>
            <span style={{ color: "#f87171" }}>
              No — {nP.toFixed(1)}% &nbsp;(
              {no.toLocaleString(undefined, { maximumFractionDigits: 0 })} DGT)
            </span>
          </div>
          <div
            style={{
              ...
              borderRadius: "100px",
              overflow: "hidden",
              background: total === 0 ? "rgba(255,255,255,0.06)" : "#f87171",
            }}
          >
            {total > 0 && (
              <div
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              ...
              >
                ...
                <path
                  strokeWidth="4"
                  strokeDasharray={`${Math.min((total / (quorumNeeded || 1)) * 100, 100)},100`}
                  strokeLinecap="round"
                  stroke={quorumMet ? "#34d399" : "#fbbf24"}
                  fill="none"
                  d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
                />
              </svg>
            ...
            >
              ...
              {quorumNeeded.toLocaleString(undefined, {
          ...
          <Section title="Actions">
              ...
              >
                <button
                  ...
                  style={{
                    ...
                    color: canVote ? "#34d399" : "rgba(52,211,153,0.4)",
                    fontFamily: "'Space Mono',monospace",
                    fontSize: "11px",
                    fontWeight: 700,
                ...
                <button
                  ...
                  style={{
                    ...
                    color: canVote ? "#f87171" : "rgba(248,113,113,0.4)",
                    fontFamily: "'Space Mono',monospace",
                    fontSize: "11px",
                    fontWeight: 700,
                    cursor: canVote ? "pointer" : "not-allowed",
            ...
            <button
              ...
              style={{
                ...
                color: canExecute ? "#fbbf24" : "rgba(245,158,11,0.3)",
                fontFamily: "'Space Mono',monospace",
                fontSize: "11px",
                fontWeight: 700,
              ...
              onMouseLeave={(e) => {
                e.currentTarget.style.background = canExecute
                  ? "rgba(245,158,11,0.15)"
                  : "rgba(245,158,11,0.05)";
              }}
            >
