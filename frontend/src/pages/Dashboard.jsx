  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "20px" }}>
        <div className="dgt-skeleton" style={{ width: "48px", height: "20px" }} />
        <div className="dgt-skeleton" style={{ width: "80px", height: "20px", borderRadius: "100px" }} />
      </div>
      <div className="dgt-skeleton" style={{ width: "60%", height: "14px", marginBottom: "10px" }} />
      <div className="dgt-skeleton" style={{ width: "100%", height: "6px", borderRadius: "100px", marginBottom: "20px" }} />
      <div style={{ display: "flex", gap: "10px" }}>
        <div className="dgt-skeleton" style={{ flex: 1, height: "38px", borderRadius: "8px" }} />
        <div className="dgt-skeleton" style={{ flex: 1, height: "38px", borderRadius: "8px" }} />
      </div>
    </div>
  );
  ...
  return (
    <button
      ...
      style={{
        ...
        padding: "10px 18px",
        borderRadius: "8px", border: isPrimary ? "none" : "1px solid rgba(255,255,255,0.08)",
        background: isPrimary ? "var(--color-primary, #F59E0B)" : "transparent",
        color: isPrimary ? "#000" : "#666",
        fontFamily: "'Space Mono', monospace",
        fontSize: "10px", fontWeight: isPrimary ? 700 : 400,
      ...
      onMouseEnter={e => { if (!disabled) { e.currentTarget.style.transform = "translateY(-2px)"; if (!isPrimary) e.currentTarget.style.borderColor = "rgba(255,255,255,0.16)"; } }}
      onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; if (!isPrimary) e.currentTarget.style.borderColor = "rgba(255,255,255,0.08)"; }}
    >
      {children}
    </button>
  );
  ...
  return (
    <div style={{ position: "relative", width: "200px" }}>
      ...
      <input
        ...
        onFocus={e => { e.currentTarget.style.borderColor = "rgba(245,158,11,0.35)"; e.currentTarget.style.background = "rgba(245,158,11,0.02)"; }}
        onBlur={e => { e.currentTarget.style.borderColor = "rgba(255,255,255,0.07)"; e.currentTarget.style.background = "rgba(255,255,255,0.02)"; }}
      />
    </div>
  );
}
...
function EmptyState({ icon, title, subtitle, action }) {
  return (
    <div style={{
      ...
      {subtitle && (
        <p style={{ fontFamily: "'Space Mono', monospace", fontSize: "10px", color: "#444", margin: 0, letterSpacing: "0.05em" }}>
          {subtitle}
        </p>
      )}
      {action}
...
export default function Dashboard() {
  ...
  const activeCount = proposals.filter(p => now >= p.startTime && now <= p.endTime && !p.canceled).length;
  ...
  let filtered = proposals.filter((p) => {
    ...
    if (searchQuery) {
      ...
      if (!p.id.toString().includes(q) && !p.target.toLowerCase().includes(q)) return false;
    }
    return true;
  });
  ...
  filtered.sort((a, b) => {
    ...
    if (sortBy === "Most Votes") {
      return (parseFloat(b.yesVotes) + parseFloat(b.noVotes)) - (parseFloat(a.yesVotes) + parseFloat(a.noVotes));
    }
    return b.id - a.id;
  });
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: "rgba(245,158,11,0.05)", filter: "blur(100px)",
        pointerEvents: "none", zIndex: 0,
      }} />

      {/* ─── Page header ─── */}
      <div className="dgt-fade-up" style={{
        ...
        <div>
          <span style={{
            ...
            color: "var(--color-primary, #F59E0B)", display: "block", marginBottom: "8px",
          }}>Protocol</span>
          <h1 style={{
            fontFamily: "'Cormorant Garamond', serif",
            ...
            Governance <em style={{ fontStyle: "italic", color: "var(--color-primary, #F59E0B)" }}>Dashboard</em>
          </h1>
          <p style={{ fontFamily: "'Space Mono', monospace", fontSize: "11px", color: "#444", margin: 0, letterSpacing: "0.05em" }}>
      ...
      {account && proposals.length > 0 && (
        <div className="dgt-fade-up-d1" style={{
          display: "flex", gap: "12px", flexWrap: "wrap",
          marginBottom: "36px", position: "relative", zIndex: 1,
        }}>
          <StatCard label="Total Proposals" value={proposals.length} />
          <StatCard label="Active" value={activeCount} valueColor="var(--color-success, #4ade80)" />
          <StatCard label="Executed" value={executedCount} valueColor="var(--color-primary, #F59E0B)" />
        </div>
      )}
      ...
      <div className="dgt-fade-up-d2" style={{
          ...
          {account && proposals.length > 0 && (
            <div style={{ marginBottom: "16px" }}>
              <span style={{
                ...
                letterSpacing: "0.14em", textTransform: "uppercase", color: "#444",
              }}>
                {filtered.length} proposal{filtered.length !== 1 ? "s" : ""} · {filter} · sorted by {sortBy}
              </span>
            </div>
          )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            {!account ? (
              ...
              <EmptyState
                ...
                subtitle={filter === "All" ? "Be the first to create a governance proposal" : "Try a different filter"}
                action={filter === "All" && (
                  <button
        ...
        {/* Sidebar */}
        <div style={{ width: "300px", flexShrink: 0, display: "none" }} className="lg-sidebar">
          <UserPanel />
        </div>
      </div>
